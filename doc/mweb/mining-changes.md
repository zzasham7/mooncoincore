...
## Activation Status (Mooncoin Mainnet)

- MWEB is **not active** on current Mooncoin mainnet.
- This document is a **readiness guide** for pools and operators, not a current mandatory mining change.
- Current priority is miner/pool/node migration to the Mooncoin Core v0.21.4 line.
- If ecosystem readiness is achieved, MWEB activation can be scheduled in a later release window (for example, a planned v0.21.5 activation process) with explicit height announcement and coordination.

## When This Applies

The procedure below applies only after MWEB activation is announced and reached on-chain.

In the Mooncoin Core v0.21.x code line, `getblocktemplate` includes support for mining blocks with MWEB data. `getblocktemplate` does not provide pool software fine-grained control over which MWEB transactions are included, and instead builds MWEB and integrating transactions for you. Pool-side changes are typically minimal.
...
The reason for (3) is that the HogEx commits to the MWEB data, moves pegged-in coins to the HogAddr (address holding all MWEB coins), and sends pegout coins to the addresses specified by MWEB pegout transactions. Any mismatch between MWEB data and the HogEx transaction causes consensus failure for that block.
