...
There are 2 ways that light clients can identify coins belonging to a wallet:

## 1. Server-Side Filtering

[BIP37 - Bloom Filters](https://github.com/bitcoin/bips/blob/master/bip-0037.mediawiki): Light wallets build a bloom filter for a list of addresses belonging to them, and share it with a full node. That full node then checks for outputs that _probabilistically_ belong to the wallet.
...
For those who don't have a trusted full node to connect to, we can use something similar to client-side block filtering. More precisely, we can perform client-side _output_ filtering. Though no method has yet been designed for building filters as compact as those defined in [BIP158 - Compact Block Filters for Light Clients](https://github.com/bitcoin/bips/blob/master/bip-0158.mediawiki), nodes can provide light clients with _compact_ MWEB outputs, allowing them to avoid downloading the large rangeproofs associated with each output. Following the process defined in [LIP-0006](https://github.com/DavidBurkett/lips/blob/LIP0006/LIP-0006.mediawiki), light clients can download and check UTXOs from multiple nodes _in parallel_, reducing the privacy and verifiability shortcomings of server-side filtering.
