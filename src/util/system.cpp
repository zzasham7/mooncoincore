...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MooncoinData";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/MooncoinData";
#else
    // Unix-like
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("Bitcoin Core") == std::string::npos) {
        ...
        size_t year_pos = strYear.find("2013");
        if (year_pos == std::string::npos) {
            year_pos = strYear.find("2011");
        }
        if (year_pos != std::string::npos) {
            strYear.replace(year_pos, 4, "2009");
