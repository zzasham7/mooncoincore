    ...
    QWidget(nullptr, f), curAlignment(0)
{
    ...
    // Draw dedicated splash artwork for startup background.
    // This avoids clipping caused by treating the app icon as splash content.
    QPixmap splashArt(":/icons/litecoin_splash");
    if (!splashArt.isNull()) {
        pixPaint.drawPixmap(QRect(QPoint(0, 0), QSize(480, 320)), splashArt, QRect(QPoint(0, 0), splashArt.size()));
    }
