...
static int GenerateMTRandom(unsigned int seed, int range)
{
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams, const uint256& prevHash)
{
    ...
    const std::string seed_hex = prevHash.ToString().substr(7, 7);
    const unsigned int seed = static_cast<unsigned int>(std::strtoul(seed_hex.c_str(), nullptr, 16));

    if (nHeight <= 100000) {
        nSubsidy = static_cast<CAmount>(1 + GenerateMTRandom(seed, 1999999)) * COIN;
    } else if (nHeight > 193076 && nHeight < 203158) {
        nSubsidy = 2519841 * COIN;
    } else if (nHeight <= 203518) {
        nSubsidy = static_cast<CAmount>(1 + GenerateMTRandom(seed, 999999)) * COIN;
    } else if (nHeight <= 250000) {
        nSubsidy = static_cast<CAmount>(1 + GenerateMTRandom(seed, 599999)) * COIN;
    } else if (nHeight <= 300000) {
        nSubsidy = static_cast<CAmount>(1 + GenerateMTRandom(seed, 349999)) * COIN;
    } else if (nHeight <= 350000) {
        nSubsidy = static_cast<CAmount>(1 + GenerateMTRandom(seed, 174999)) * COIN;
    } else if (nHeight <= 375000) {
        nSubsidy = static_cast<CAmount>(1 + GenerateMTRandom(seed, 99999)) * COIN;
    } else if (nHeight <= 384400) {
        nSubsidy = static_cast<CAmount>(1 + GenerateMTRandom(seed, 49999)) * COIN;
    }
    ...
    if (nHeight > 1099999) {
        nSubsidy = static_cast<CAmount>(std::floor(19697202017.0 / (std::floor(nHeight / 100000.0) * 100000.0))) * COIN;
    }
    if (nHeight > 1249999) {
        nSubsidy = static_cast<CAmount>(std::floor(std::floor(0.29531 * 19697202017.0) / (std::floor(nHeight / 100000.0) * 100000.0))) * COIN;
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    CAmount blockReward = nFees + GetBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(), block.hashPrevBlock);
    ...
    EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    // Mooncoin historical note:
    // bit0/bit1 were historically used for CSV/SegWit signaling on legacy
    // branches. In this codebase these rules are enforced as buried
    // deployments, so treat bit0/bit1 as known historical signals and do not
    // surface them as "unknown".
    static constexpr int kKnownHistoricalVersionbitCsv = 0;
    static constexpr int kKnownHistoricalVersionbitSegwit = 1;
    if (!::ChainstateActive().IsInitialBlockDownload())
    {
        const CBlockIndex* pindex = pindexNew;
        for (int bit = 0; bit < VERSIONBITS_NUM_BITS; bit++) {
            if (bit == kKnownHistoricalVersionbitCsv || bit == kKnownHistoricalVersionbitSegwit) {
                continue;
            }
...
bool IsWitnessEnabled(const CBlockIndex* pindexPrev, const Consensus::Params& params)
{
    ...
    // Historical Mooncoin mainnet compatibility:
    // legacy chain accepted pre-1793913 history without strict segwit enforcement.
    if (params.hashGenesisBlock == uint256S("0xbd687cbbac4842724d4b1dfd2e1a6ce35d765db05635b3ebd3810ba66ac5aee9") &&
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    // Mooncoin historical chain compatibility:
    // v2/v3 version enforcement was not historically applied on mainnet.
    // Keep only the v4 gate (BIP65) to align with v0.17 behavior.
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    // For historical Mooncoin compatibility, only enforce this once witness
    // rules are considered active for the current chain state.
